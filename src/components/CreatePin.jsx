...
const acceptedTypes = ['jpeg', 'png', 'svg', 'gif', 'tiff'];
					...
					<div className="flex flex-col">
						<div>
							...
							<select onChange={e => setCategory(e.target.value)} className="outline-none w-4/5 text-base border-b-2 border-gray-200 p-2 rounded-md cursor-pointer">
								...
								{categories.map(cat => (
									<option key={cat.name} className="text-base border-0 outline-none capitalize bg-white text-black" value={cat.name}>{cat.name}</option>
								))}
							</select>
						</div>
